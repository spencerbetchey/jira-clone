...
function TicketHistory({ ticketId }) {
  ...
  if (loading) {
    return <div className="text-sm text-gray-400">Loading history...</div>
  }

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
      <h2 className="text-sm font-semibold text-gray-800 mb-4">Activity</h2>
          ...
          {history.map((entry) => (
            <div key={entry.id} className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <span className="text-xs text-gray-400">{formatTimeAgo(entry.changed_at)}</span>
              </div>
            </div>
          ))}
        </div>
